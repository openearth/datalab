...
class BootstrapDatetimePickerWidget(forms.DateTimeInput):

    class Media:
        class JsFiles(object):
            def __iter__(self):
                yield 'js/moment-with-langs.min.js'
    ...
    def __init__(self, attrs=None, format=None, options=None, div_attrs=None, icon_attrs=None):
        ...
        super(BootstrapDatetimePickerWidget, self).__init__(attrs, format)
...
class TerminalWidget(forms.Widget):
    def render(self, name, value, attrs=None):
        if value is None: value = ''
        final_attrs = self.build_attrs(attrs, name=name)
        return format_html('<br><br><pre{0}>\r\n{1}</pre>',
                           flatatt(final_attrs),
                           force_text(value))

    class Media:
        css = {
            'all': ('css/processing.css',),
        }
        js = ('js/processing.js', )
